...
よくあるユースケースとしてはチャットでコードの書き方を質問して、サンプルコードを生成してもらい、コピー＆ペースト＆統合して利用するです。そのほか、エラーが発生した際に、エラーログをチャットで伝えて、エラーの意味や解決方法を質問し、回答をもとに修正を行うスタイルも含まれます。
...
（後半パートで実際にデモしながら解説します。特にSpec駆動のツールは使いません）
...
GitHub Copilotでは役割を直接チャットで指示する他に、`.github/copilot-instructions.md` ファイルに記述する、`.github/promnts`以下によく使うプロンプトを用意する、モードを追加して役割を指定するなど、様々な方法でプロフィールを設定できます。これにより、AIエージェントは一貫したルールに基づいて行動する確率が高まります。
...
補足：上記の例は、計画を短期記憶として会話のコンテキストウインドウに保持し、後続の行動に活用します。Spec駆動開発の場合は、仕様リストや設計指針や計画などを、外部ファイルに保存し長期記憶として計画を保持します。
