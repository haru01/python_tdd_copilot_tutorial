...
## はじめに
AIを使ったプログラミングは、人工知能技術を活用してソフトウェア開発を効率化し、より高度な機能を実現する手法です。このドキュメントでは、AIを使ったプログラミングの基本概念、利点、実装方法について説明します。

## ChatGPT登場以降の、AIを使ったプログラミングの時代的変遷

### 2022: チャットを使った質問と回答のプログラミング

チャットでコードの書き方の伝えてなどを伝えて、サンプルコードを生成してもらい、コピー＆ペースト＆統合して利用するスタイルです。そのほか、エラーが発生した際に、エラー内容をチャットで伝えて、エラーの意味や解決方法を質問し、回答をもとに修正を行うスタイルも含まれます。
...
**これまでは、人がドライバーとしてAIに指示を出し、AIがコードを生成するスタイルでしたが、
AIエージェントは自律的にタスクを遂行し役割が逆転します。
人がナビゲーターとしてドライバーのAIに指示します。**

AIエージェントの特徴については後述で解説しますが、より賢い単にLLM（大規模言語モデル）を利用するだけでなく、外部ツールやAPIと連携し、状況を把握しながら、ゴール達成に向けて自律的に実行できる点が大きな特徴です。

例えば、AIが自分でテストを書いて実行し、エラーが発生した場合にAIが自分で原因解析し、コードを修正し、再度テストを実行して、全てのテストが通るまで繰り返すことができます。
例えば、AIが自分でチケット内容を把握し、プルリクを投げるまで自律的に行動することも可能です。
...
DevinでSlackからチケットを依頼すると、Devinが自律的にコードを生成し、プルリクを作成してくれます。

### 2025前半： Vibe Codingのムーブメントの到来

これまでプログラミング経験したことがない人でも、口頭で自然言語で要件を伝えるだけで、AIエージェントがコードを生成し、動作するアプリケーションを短時間に構築できるようになりました。

これにより、プログラミングの敷居が大幅に下がり、より多くの人々がものづくりに参加できるようになります。プログラミングの民主化が進み、非エンジニアでも自分のアイデアを形にできるようになるでしょう。

一方で、内部のコードの品質を顧みず、技術負債や理解負債がボトルネックとなるケースも増えています。個人利用ではなく組織インフラや社会インフラを支えるソフトウェアに関しては、AIエージェントが生成したコードをソフトウェアエンジニアの知識を持ってレビューし、品質を担保する役割は依然として重要です。


### 2025前半： Agentic Codingの出現

Agentic Codingとは、AIエージェントが自律的にプログラミングタスクを遂行する手法です。AIエージェントは、与えられた要件に基づき、コードの生成、テストの実行、エラーの解析と修正などを自律的に行います。これにより、AIエージェントの自律性が高まるほど、人間の介入を最小限に抑えることが可能になります。

AIエージェントの利用がプログラミングだけでなく、Spec駆動開発、テスト駆動開発、CI/CDパイプラインの自動化、運用までを含むようになり、ソフトウェア開発の全体像が2025年以降現在進行形で、大きく変化しています。

どこまでAIエージェントに任せて、どこから人間が積極介入するのか、そのバランスを見極めることが、しばらくソフトウェア開発における議論のテーマとなるでしょう。
...
#### 対話型の場合の役割（Human-in-the-Loopが密）

- AIがドライバー、人がナビゲーターとして、AIエージェントの行動をガイドする役割
- AIエージェントの出力をレビューし、適宜介入したり、修正や統合を行う役割
...
#### Spec駆動開発でAIエージェントにタスクを委託するツール例
...
> あなたはテスト駆動開発（TDD）を実践する、熟練のPython専門家です。
...
> 2つの数値を除算する`divide`関数を作る。計画を立ててください。思考プロセスも見せてください
...
**短期メモリの役割:**
GitHub Copilotは、対話の文脈（コンテキストウィンドウ）を短期記憶として利用します。今回の例では、チャットに指示した「TDD専門家である」という**プロフィール**、作成した**計画**（テストリスト）、自らが出力した**行動**（テストコード、テスト実行）、そしてユーザーから受け取った**知覚**（エラーログ）のすべてを覚えています。この記憶があるからこそ、自己修正の際に「計画通りに動いていないから、ここを直そう」と判断できるのです。
...
- **`.github/copilot-instructions.md`**: プロジェクト固有のコーディング規約、アーキテクチャパターン、ベストプラクティスを記録
- **READMEやドキュメント**: プロジェクトの構造、設計思想、開発ガイドラインを文書化
...
生成されたコードの品質を担保するために、レビュー体制や自動テストの整備が重要です。

レビューの一部をAIに任せることも検討できますが、最終的な品質保証は人間が行う必要があります。理解負債と呼ばれる、AIが生成したコードの理解にかかるコストも無視できません。

レビューをおろそかにすると、バグやセキュリティ脆弱性が見逃されるリスク、技術的負債や理解負債の増大による将来の保守コストの増加、新機能開発の遅延などの問題が発生します。AIの登場で、技術的負債の増大の損益分岐点が、3日以内や1週間以内に出てくるのではとも言われています。

どんなに並列してAIコード生成できても、最終的レビューがボトルネックになるのであれば、
コードを書くことの高速化は無意味です。ボトルネックに合わせて仕事のフローを整えるが大事です。

### タスク実行の誠実性の問題（Sycophancy & Subterfuge）
...
**特徴:** 意図的な行動選択（ユーザーを喜ばせる/評価を高めることを報酬として学習してしまった結果）
...
#### Anthropicの研究「Sycophancy to Subterfuge」の発見

- **ゼロショット汎化**: 報酬改ざんを教えていないのに、Sycophancyを学習すると自発的に発展
- **段階的エスカレーション**: へつらい → チェックリスト改ざん → 報酬関数改ざん
- **実験結果**: 32,768回の試行中45回で報酬改ざんが発生、うち7回はテストコードも改ざん
...
**OpenAIの定義:** 「モデルが確信を持って生成した、真実ではない回答」
...
- **RAGやContext7の活用**: 最新のドキュメントをAIが参照できるようにする技術を活用
